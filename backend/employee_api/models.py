...
class MedicalRecord(models.Model):
    ...
    # UPDATED: Stores medicines with quantity and timings
    # e.g., {"tablets": {"Paracetamol": {"quantity": 10, "timings": ["maf", "naf"]}}}
    medication_details = models.JSONField(default=dict, blank=True)
    
    # NEW: Stores a simple list of prescribed test names
...
class DiagnosticTest(models.Model):
    ...
    is_sent_to_patient = models.BooleanField(default=False) # Doctor/Admin sends this
    is_paid = models.BooleanField(default=False) # Patient pays for this

    created_at = models.DateTimeField(auto_now_add=True)
    def __str__(self): return f"Test '{self.test_name}' for {self.patient.username}"
    class Meta: ordering = ['-created_at']

# --- NEW MedicationBill Model ---
